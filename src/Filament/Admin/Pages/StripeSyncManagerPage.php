        ...
        return $table
            ...
            ->columns([
                TextColumn::make('syncable.name')
                    ->label('Item')
                    ->description(fn (?StripeSyncLog $record): ?string => $record?->syncable_type ? class_basename($record->syncable_type) : null
                    )
                    ->placeholder('N/A')
                    ->searchable()
                    ->toggleable(),
                ...
                TextColumn::make('error_message')
                    ->label('Error')
                    ->color('danger')
                    ->visible(fn (?StripeSyncLog $record): bool => $record?->status === 'failed'
                    )
                    ->wrap()
                    ->toggleable()
                ...
                TextColumn::make('stripe_id')
                    ->label('Stripe ID')
                    ->visible(fn (?StripeSyncLog $record): bool => $record?->status === 'success'
                    )
                    ->toggleable()
                    ->placeholder('N/A'),
            ])
            ...
            ->actions([
                Action::make('retry')
                    ...
                    ->visible(fn (?StripeSyncLog $record): bool => $record?->status === 'failed' &&
                        $record?->syncable instanceof Product
                    )
                    ->action(function (StripeSyncLog $record): void {
                        if (! $record->syncable instanceof Product) {
                            Notification::make()
